...
---We used pytorch transformers framework in our project . For training we used Pytorch Lightning and for the utility we used hydra. ---
...
--- Initially start by isolating your project from system wide environment and create a virtual environment.
For managing project specific dependencies we created a requirements.txt file which will help installing all the required packages dependecies.
Any new comer if follows above mentioned steps will be able to successfully get an exact copy of our environment.  ---
...
--- We have tried incorporating flake8 through the tests . Code quality and formatting becomes very important in the large scale projects because of multiple people from different departments work on same project and view and edit the code . So to maintain the consistency ,readability becomes simpler for anyone including the newcomer. ---
...
--- In total we have implemented 3 tests. Pytests for the data and training part of the project. test_data to check if the data generation is as expected and test_train to check if the training is proceeding as expected and we get appropriately tokenized data.
We also include flake8 test to implement the uniform formatting across the project---
...
--- The total coverage of the code for test_data is 85% and test_train is 80% . We have included the coverage reports in the tests folder in the src.No, even if the total coverage of the code is 100% it does not guarantee that it is error free. Some of the reasons include incomplete test scenarios like lack of including edge cases , not covering of all data variations. Also it depends on the way the tests are written. ---
...
--- Yes we used branches that is each member pushed the code as seperate branch and later we merged it to the main branch.They help in Parallel development of the repository and isolation ie one developers work does not affect another.Pull requests are usefull in this open source world as it helps in code review and much more.  ---
...
--- We used dvc in our project which helped us in the data sharing part and keeping track of the data. ---
...
--- Image is in the figures folder 
---
